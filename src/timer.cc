...
void init_timer(void) { QueryPerformanceFrequency(&frequency); }
...
std::string Timer::summery(void) {
  Timer::instance().whole_program.total_run_time =
      get_current_time() - Timer::instance().whole_program.begin_time;
  ...
  for (it = instance().datas.begin(); it != instance().datas.end(); it++) {
    ...
    ss.width(percentage_size - sizeof("%"));
    ss << std::right
       << int(100000 * it->second.total_run_time / tatol_running_time) / 1000.0
       << " % |" << endl;
  }
